...
void ppp_task(void *arg) {
    ...
    if (uart_queue == NULL) {
        ESP_LOGE(TAG, "uart_queue is NULL! Aborting task.");
        vTaskDelete(NULL);
    }
    ...
    while (1) {
        if (xQueueReceive(uart_queue, (void *)&event, portMAX_DELAY)) {
            switch (event.type) {
                case UART_DATA: {
                    int len = uart_read_bytes(MODEM_UART_NUM, buf, event.size, portMAX_DELAY);
                    if (len > 0) {
...
void init_uart_modem() {
   ...
   // uart_driver_install(MODEM_UART_NUM, BUF_SIZE * 2, 0, 0, NULL, 0);
    ...
    uart_driver_install(MODEM_UART_NUM, BUF_SIZE * 2, BUF_SIZE * 2, 20, &uart_queue, 0);
}
...
void ppp_task(void *arg) {
    ...
    while (1) {
        int len = uart_read_bytes(MODEM_UART_NUM, buf, BUF_SIZE, pdMS_TO_TICKS(500));
        if (len > 0) {
            pppos_input_tcpip(ppp, buf, len);
        }
    }
...
void init_wifi_ap() {
    ...
    esp_netif_dhcps_option(ap_netif, ESP_NETIF_OP_SET, ESP_NETIF_DOMAIN_NAME_SERVER, &dns.ip.u_addr.ip4.addr, sizeof(ip4_addr_t));
    ESP_LOGI(TAG, "DHCP server DNS set to 8.8.8.8");
...
void app_main(void) {
    ...
    xTaskCreatePinnedToCore(ppp_task, "ppp_task", 12288, NULL, 5, NULL, 0);
}
